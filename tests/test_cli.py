...
class FakeInteractivePaneManager:
    def __init__(self) -> None:
        self.sent: list[tuple[int, str]] = []

    def summary(self) -> dict:
        return {"total_windows": 1, "done": 1}

    def get_text(self, pane_id: int) -> str:
        return f"output of pane {pane_id}"
...
def test_interactive_loop_quit(monkeypatch):
    ...
    pane_mgr = FakeInteractivePaneManager()
    cli._interactive_loop(pane_mgr, SimpleNamespace(subtasks=[]))


def test_interactive_loop_inspect(monkeypatch, capsys):
    inputs = iter(["inspect 42", "q"])
    monkeypatch.setattr("builtins.input", lambda _prompt: next(inputs))
    pane_mgr = FakeInteractivePaneManager()
    cli._interactive_loop(pane_mgr, SimpleNamespace(subtasks=[]))
    captured = capsys.readouterr()
    assert "output of pane 42" in captured.out


def test_interactive_loop_send(monkeypatch):
    inputs = iter(["send 42 hello world", "quit"])
    monkeypatch.setattr("builtins.input", lambda _prompt: next(inputs))
    pane_mgr = FakeInteractivePaneManager()
    cli._interactive_loop(pane_mgr, SimpleNamespace(subtasks=[]))
    assert (42, "hello world") in pane_mgr.sent
